...
例２　　「チョコとポテトチップ下さい」（自然言語のみによるリクエスト）
...
```
顧客　　「チョコとポテトチップ下さい」
ロボット「チョコはダークチョコ，ミルクチョコ，イチゴチョコがあります．ミルクチョコで良いですか？」
顧客　　「Yes」
ロボット「ポテトチップは，塩味，コンソメ味，BBQ味があります．コンソメ味で良いですか？」
顧客　　「Yes」
ロボット「サイズは小さい物，大きい物がありますが，小さいサイズで良いですか？」
顧客　　「No」
ロボット「わかりました．ミルクチョコと，コンソメ味のポテトチップ小サイズをお持ちします」
...
セッションごとに得点を累計し，参加チームの中で最も高い合計得点を100点に正規化した数値を，各チームの"Interactive Customer Service "の得点とする。
...
| 5回以上の回数(n回)の質問をする | (n-5) 点減点，最大-20点 |
| ギブアップ／システムエラー     | そのセッションの得点を0 とする |

なお，ギブアップは任意のタイミングで行うことができる．その場合，ギブアップしたセッションの次のセッションに進み，競技を継続する．競技そのものをギブアップする場合には，競技主催者にその旨を宣言する．システムエラーが発生し，競技を継続不可能になった場合，エラーが発生した次のセッションから再開することができる．
ギブアップは，ロボットのソフトウェアが自ら判断してギブアップしても良いし，オペレーターが強制的にギブアップを宣言しても良い．前者のソフトウェアによるギブアップの場合には，自動的に次のセッションに進む．後者のオペレータによるギブアップの場合には，次のセッションから再開するためのソフトウェアの設定を競技運営側が行い，再開する．
ギブアップした場合もしくはエラーが発生した場合には，そのタイミングまでに獲得した点数に寄らず，該当セッションの得点を0点とする．

競技者は以下のようなAPIを利用してロボットの対話機能を制御する．APIは python およびROSで提供予定であり，詳細なAPIは2024年2月中旬～下旬に公開予定である
...
## 競技タスク２  Future Convenience Store Design
...
評価をする審査委員は，競技会主催者が指名して決定し，最低でも４名の審査委員で評価する．審査委員の中には英語Naitive ではない委員がいる可能性があるため，その場合には，質疑応答の翻訳補助を競技会主催者が行う．
